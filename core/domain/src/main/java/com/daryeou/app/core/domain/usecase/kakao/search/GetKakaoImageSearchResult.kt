...
/**
 * Use case to get the image search result from Kakao API
 */
class GetKakaoImageSearchResult @Inject constructor(
     ...
     * @param page The page number, must be smaller than 80
    ...
    operator fun invoke(
        ...
        // Check size is not greater than 80
        if (size > 80) {
            throw IllegalArgumentException("Size must be less than or equal to 80")
        }
        return kakaoSearchRepo.getKakaoImageSearchList(query, sort, page, size)
    }
}